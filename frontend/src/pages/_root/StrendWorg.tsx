...
export default function StrendWorg(props) {
    ...
    const handleClick = (buttonId, type) => {
        setLastClicked(buttonId);
        setType(pokemonTypes[type])
    };
    ...
    return (
        <div>
                ...
                {props.pokemon.types.map((item, index) => {
                    return (
                        <img
                            ...
                            onClick={() => handleClick(index, item.type.name)}
                            src={pokemonTypes[item.type.name].image}
                            style={{
                                opacity: lastClicked === index ? 1 : 0.35
                            }} />
                    )
                })}
            </div>
            <div className='flex flex-col my-5 gap-3'>
                <h1 className='text-white text-2xl mt-3'>Strength</h1>
                <div className='flex gap-2'>
                    {type.strength.map((item, index) => (
                        <img title={formatUrlType(item)}  className='w-10' src={item} key={index} /> // Adicionando key para evitar warning
                    ))}
                </div>
            </div>

            <div className='flex flex-col my-5 gap-3'>
                <h1 className='text-white text-2xl mt-3' >Weakness</h1>
                <div className='flex gap-2'>
                    {type.weakness.map((item, index) => (
                        <img title={formatUrlType(item)}  className='w-10' src={item} key={index} /> // Adicionando key para evitar warning
                    ))}
                </div>
            </div>
            ...
            <div className='flex flex-col my-5 gap-3'>
                <h1 className='text-white text-2xl mt-3' >Resistance</h1>
                <div className='flex gap-2'>
                    {type.resistance.map((item, index) => (
                        <img title={formatUrlType(item)}  className='w-10' src={item} key={index} /> // Adicionando key para evitar warning
                    ))}
                </div>
            </div>
            ...
            <div className='flex flex-col my-5 gap-3'>
                <h1 className='text-white text-2xl mt-3' >Vulnerable</h1>
                <div className='flex gap-2'>
                    {type.vulnerable.map((item, index) => (
                        <img tabIndex={0} title={formatUrlType(item)} className='w-10' src={item} key={index} /> // Adicionando key para evitar warning
                    ))}
                </div>
            </div>
        </div>
...
// {formatterArrayDetails(pokemonTypesprops.pokemon.types[0].type.name)}